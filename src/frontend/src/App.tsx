...
function calcSecondary(d: FormData) {
  const food = (Number(d.foodDrink) || 0) * 0.015;
  const clothes = (Number(d.clothes) || 0) * 0.02;
  const electronics = (Number(d.electronics) || 0) * 0.025;
  const lifestyle =
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Enter average km per week for each mode
          </p>
        </div>
      </div>
      <FieldRow
  ...
  const clsConfig = {
    ...
    medium: {
      ...
      impacts: [
        ...
        "Your footprint is higher than India's average (~1.9 t/year) but close to the global average (~4.7 t/year), indicating a moderate lifestyle impact.",
        "With targeted changes in energy use and transport, you can significantly reduce your emissions.",
      ],
      tips: [
          ...
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
            ...
            {formData.flights.length === 0 ? (
              ...
              <div className="space-y-1 text-muted-foreground">
                {formData.flights
                  .filter(
                    (fl) => Number(fl.trips) > 0 && Number(fl.distanceKm) > 0,
                  )
                  .map((fl, i) => (
                    <p key={`flight-${i}-${fl.from}-${fl.to}`}>
                      {fl.from || "Origin"} → {fl.to || "Destination"} ·{" "}
                      {fl.flightClass} · {fl.trips} trip(s)
                    </p>
                  ))}
              </div>
            )}
...
function MethodologySection() {
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: "Classify the total into Low (<150 kg), Medium (150–500 kg), or High (>500 kg) per month.",
